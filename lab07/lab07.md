...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - Sequence number 0
   - В заголовке имеется флаг SYN
   - ![alt-text](images/syn.png)
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - 0
   - 1
   - По следующему порядковому номеру
   - В заголовке выставлены оба флага SYN и ACK
   - ![alt-text](images/ack.png)
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - Если сегмент с POST первый, то 6 сегментов начиная с него: 1, 618, 755, 2215, 3675, 5135
   - 3.152005000, 3.153065000, 3.153688000, 3.153695000, 3.153700000, 3.153705000 в секундах с начала запуска захвата (или же можно отнять от всех первое значение если считать первый сегмент началом соединения)
   - По одной из картинок выше можно увидеть, что только 2 сегменты были подтверждены: 618 и 755. 3.298507000 и 3.298508000 секунд соответственно
   - 0.145442 с для 618 и 0.14482 с для 755
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - Посмотрим на размер окна, равный 262144 байта. Из прошлого пункта, зная что средняя задержка 0.145 сек, получим $\dfrac{262144}{0.145} \approx 14.5$ мегабит в секунду
...
#### Решение
По формуле $U_{\text{отправитель}} = n\dfrac{\dfrac{L}{R}}{RTT + \dfrac{L}{R}} > 0.98$. Подставим и получим $n > 2450$
