...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - 0x6ba1eae7. Это поле необходимо, чтобы хост понимал на какой запрос пришел ответ
   - ![alt-text](images/ws1.png)
4. Хост использует DHCP-протокол, в частности, для получения IP-адреса. Но IP-адрес хоста
   ...
   - В запросе исходный адрес 0.0.0.0, а ответный - 255.255.255.255 (то есть оправился всем хостам в сети). В ответе исходный 192.168.0.1 (адрес DHCP сервера), а ответный 192.168.0.7
5. Каков IP-адрес вашего DHCP-сервера?
   - 192.168.0.1
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   вашем эксперименте?
   - IP-адреса выдаются не навсегда, а временно, чтобы неиспользующийся адрес можно было переиспользовать и кому-нибудь другому. Например у меня это 1 день
   - ![alt-text](images/ws2.png)
...
#### Решение
1. Максимальное значение функции достигается в нулях производной (либо на границах отрезка, но тут очевидно в них это просто 0). $(N \cdot p (1 - p)^{N - 1})' = N((1 - p)^{N - 1} - p (N - 1)(1 - p)^{N - 2}) = N (1 - p)^{N - 2} (1 - p - p(N - 1)) = N (1 - p)^{N - 2} (1 - pN) \implies p = \dfrac{1}{N}$
2. $\lim_{N \to +\infty}N\dfrac{1}{N}(1 - \dfrac{1}{N})^{N - 1}$. Это известный предел и он равен $e^{-1}$
...
#### Решение
1. Чтобы узел А передал информацию в кванте 5 необходимо, чтобы он не передал ничего до этого $(1 - p)^4$ и в данный момент передал именно он $p(1 - p)^3$ $\implies p(1 - p)^7$
2. Аналогично, для конкретного узла вероятность передать что-то на 4 кванте $p(1 - p)^6$. Так как эти события несовместны получим $3p(1 - p)^6$
3. Из предыдущих пунктов веротность передачи на одном кванте $4p(1 - p)^3$. Соответственно, вероятность передачи впервые на 3 кванте           
   $(1 - 4p(1 - p)^3)^2(4p(1 - p)^3)$ 
4. Из прошлой задачи при $N = 4$ эффективность равна $4p(1 - p)^3$
