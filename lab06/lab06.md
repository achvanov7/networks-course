...
1. Выберите один UDP-пакет. По этому пакету определите, сколько полей содержит UDP-заголовок.
   - 4: Source Port, Destination Port, Lenght, Checksum
2. Определите длину (в байтах) для каждого поля UDP-заголовка, обращаясь к отображаемой
   информации о содержимом полей в данном пакете.
   - Каждый по два байта
3. Значение в поле Length (Длина) – это длина чего?
   - Длина всего пакета (поля + сами данные) в байтах
4. Какое максимальное количество байт может быть включено в полезную нагрузку UDP-пакета?
   - Чисто теоритически, так как Length это два байта, она может быть до $2^{16} - 1 = 65535$. И 8 байт всегда уйдут на заголовок, поэтому $65527$ байт
5. Чему равно максимально возможное значение номера порта отправителя?
   - Аналогично, порт может быть от $0$ до $65535$
...
7. Проверьте UDP-пакет и ответный UDP-пакет, отправляемый вашим хостом. Определите
   ...
   - Те же самые, но в другом порядке
...
#### Скрины
- Сервер ![alt-text](images/fz1.png)
- Подключение юзером ![alt-text](images/fz2.png)
- Coздание папки ![alt-text](images/fz3.png)
- Скачивание папки ![alt-text](images/fz4.png)
- Удаление на сервере и загрузка папки ![alt-text](images/fz5.png)
