...
![alt-text](images/big_text_received.png)
...
#### Решение
Размер окна увеличевается с $\dfrac{W}{2}$ до $W$ за $\dfrac{W}{2}RTT$. Вспомним, что средняя пропускная спобоность это $\dfrac{3W}{4 RTT}$. А значит $T$ это функция от средней пропускной способности TCP
...
#### Решение
Пусть $T$ время за которое окно увеличилось с $\dfrac{W}{2}$ до $W$. Тогда $L = \dfrac{1}{T}$. Но также оно увеличивается в $1 + \alpha$ раз каждые $RTT$ сек. Тогда $T$ это просто $\lceil \log_{1 + \alpha}{2} \rceil$. Или же $L = \dfrac{1}{\lceil \log_{1 + \alpha}{2} \rceil}$, что не зависит от $W$
