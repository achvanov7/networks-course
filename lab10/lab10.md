...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Identification и Header Checksum. Time to Live изменятеся не всегда, но периодично, каждые 3 пакета
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Source и Destination Address, Protocol, Version, Header Length, Flags
      - Не должны меняться поля, те которые и не меняются. Странно если было бы иначе, так как это один и тот же вид запроса с фиксированной длиной 
      - Также меняются те поля от которых мы этого и ожидаем 
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Identification имеют значения от 29505 до 29507, а TTL остается неизменным - 64.
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Total Length, Checksum и Fragment offset
