...
#### Решение
Давайте посчитаем время через которое последний пакет дойдет до приемника. Сначала он должен подождать пока дойдет его очередь $(P - 1)\frac{L}{R}$ и потом он должен пройти $N\frac{L}{R}$. Итого $(N + P - 1)\frac{L}{R}$
...
#### Решение
$\dfrac{5 Mb}{\min (R_1, R_2, R_3)} = \dfrac{5 Mb}{200 Kbit / s} = \dfrac{5 * 1000 * 8 Kbit}{200 Kbit / s} = 200$ секунд
...
#### Решение
Заметим, что одновременно передавать данные могут не более 20 пользователей. Тогда вероятность равна 
$\sum_{k = 12}^{20} {60 \choose k}0.2^k(1 - 0.2)^{60 - k} \approx 0.55$
...
#### Решение
Вспомним формулу из первого задания $(3 + \dfrac{X}{S} - 1)\dfrac{S + 80}{R} = \dfrac{(2S + X)(S + 80)}{SR} = \dfrac{2S^2 + S(160 + X) + 80X}{SR}$. Оставим только часть которая зависит от $S$. $S + 40\dfrac{X}{S}$. Возьмем производную и поймем, что минимум достигается в $S = \sqrt{40X}$. Тогда положим $S$ ближайшим целым числом к $\sqrt{40X}$
...
#### Решение
1. Так как задержка передачи это $\dfrac{L}{R}$, то общая задержка это $\dfrac{IL}{R(1 - I)} + \dfrac{L}{R} = \dfrac{L}{R}(\dfrac{I}{1 - I} + 1) = \dfrac{L}{R(1 - I)}$
2. Пусть $x = \dfrac{L}{R} \implies \dfrac{L}{R(1 - I)} = \dfrac{x}{1 - I} = \dfrac{x}{1 - ax}$
