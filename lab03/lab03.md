...
#### Решение
Пакет кодируется за $\dfrac{56 \, byte}{128 \, Kbit/s} = \dfrac{56 * 8 \, bit}{128 * 1000 \, bit/s} = 0.0035\, s = 3.5 \, ms$. А передается за $\dfrac{56 \, byte}{1 \, Mbit/s} = 0.448 \, ms$. Учитывыя задержку, получим $3.5 \, ms + 5 \, ms + 0.448 \, ms = 8.948 \, ms$
...
#### Решение
По формуле $a = \dfrac{N}{d} = \dfrac{1 \, p + 10 \, p}{10 \, ms + \dfrac{1 \, p}{100 \, p/s}} = 550 \, p/s$, где $p$ - пакет
...
#### Решение
1. Так как узким место является первый канал, то второй пакет прождет $\dfrac{L}{R_s}$ секунд, что и будет разницей в их прибытии
2. Да, потому что может возникнуть очередь, так как по второму каналу первый пакет будет передаваться медленней. Первый пакет пройдет второй канал через $\dfrac{L}{R_s} + d + \dfrac{L}{R_c}$, а второй пакет подойдет ко второму каналу $T + 2\dfrac{L}{R_s} + d$. $T + 2\dfrac{L}{R_s} + d \geq \dfrac{L}{R_s} + d + \dfrac{L}{R_c} \implies T \geq \dfrac{L}{R_s} + \dfrac{L}{R_c}$
...
#### Решение
1. $\Delta = \dfrac{85 * 10^4 \, bit}{15 \, Mbit/s} \approx 0.057 \, s$
2. $B$ и задержка нам известны, поэтому используем формулу $\dfrac{\Delta}{1 - 16\Delta} + 3 \, s \approx 3.6 \, s$
3. Время задержки в локальной сети $\dfrac{85 * 10^4 \, bit}{100 \, Mbit/s} = 0.0085 \, s$. В интернет будем ходить в 40% cлучаев поэтому итоговое время ответа $0.4 * 3.6 \, s + 0.0085 \, s= 1.4485 \, s$
