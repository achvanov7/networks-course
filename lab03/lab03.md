...
2. Да, потому что может возникнуть очередь, так как по второму каналу первый пакет будет передаваться медленней. Первый пакет пройдет второй канал через $\dfrac{L}{R_s} + d + \dfrac{L}{R_c}$, а второй пакет подойдет ко второму каналу $T + 2\dfrac{L}{R_s} + d$. $T + 2\dfrac{L}{R_s} + d \geq \dfrac{L}{R_s} + d + \dfrac{L}{R_c} \implies T \geq \dfrac{L}{R_c} - \dfrac{L}{R_s}$
